...
class GeminiBridge(BaseInferenceBridge):
    ...
    async def stream_generate(
        ...
        # Re-raise exception to engage UnifiedInferenceRouter instant fallback to local llama_rpc
        if not token_emitted and last_exception is not None and not self._generation_cancelled:
            raise RuntimeError(f"Gemini bridge failed across all endpoints: {last_exception}")
