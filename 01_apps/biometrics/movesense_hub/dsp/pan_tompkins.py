...
Implements:
1. Pan-Tompkins 1985 Real-Time QRS Detection (Bandpass, 5-pt Derivative, Squaring, MWI, Dual-Threshold Peak Detection)
2. Kamath et al. 2004 20% Clinical RR Artifact Filter
3. Short-term Heart Rate Variability RMSSD (microsecond precision)
4. Vectorized 120s Rolling Detrended Fluctuation Analysis (DFA-alpha1, scale s in [4, 16] beats)
5. Strict Rule #0 Zero-Mock Compliance: returns WAITING_FOR_SENSOR / None when sensors are absent.
"""
...
class PanTompkinsQRSDetector:
    ...
    def bandpass_filter(self, ecg_signal: Sequence[float]) -> List[float]:
        ...
        n = len(ecg_signal)
        if n < 8:
            return [float(x) for x in ecg_signal]
...
class MovesenseECGPipeline:
    ...
    def process_raw_ecg_window(
        ...
        """
        from .hemodynamics_bp import calculate_hemodynamics_bp
        from .zone2_coaching import classify_zone2_alignment
