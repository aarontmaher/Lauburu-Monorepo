...
| **Release / QA Gate** | shared (Aaron + Agent + coder per build phase) | Active release-gate state — versionCode in flight, EAS build status, Play Internal upload state, TestFlight processing state, installed-device audit verdict. Each row tracks ONE build's journey through the rule-8 four-status sequence. Distinct from the Audit queue (which produces evidence) and the Human Approval queue (which consumes Aaron decisions): this queue holds the BUILD-IN-FLIGHT state and binds the other two together. | rules 7 / 8 / 9 + `INSTALLED_DEVICE_QA_RELEASE_GATE` |
| **Data / Readiness Evidence** | shared (Codex primary for ingestion; Agent for confirmation) | Health-source artefacts (Apple Health / Health Connect samples), journal events + dose periods, manual logs, lactate / DEXA / blood-test entries, AI-cached research artefacts (rule 23), athlete-memory snapshots, technique evidence rows. Distinct from App Functionality (which ships UI) and Audit (which validates UI): this queue holds the **evidence corpus** that Grappling Readiness + style evolution + verified mastery layers all consume. | rules 9 / 18 / 22 / 23 + `MCP_MEMORY_ARCHITECTURE_SPEC` + `JOURNAL_IMPORT_NORMALIZE_INSIGHTS_SPEC` |
