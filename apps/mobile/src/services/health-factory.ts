...
function makeInlineHealthKitService(hk: any): IHealthService {
  ...
  const READ = [
    ...
    // Dietary nutrition types — read so the Nutrition card can ingest
    // from Apple Health when any third-party source (Cronometer export,
    // MyFitnessPal, HealthKit manual entry) writes these. Keeps
    // Cronometer direct optional rather than required.
