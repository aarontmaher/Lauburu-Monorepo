...
============================================================
Executes a rigorous 4-turn deliberative debate among:
  1. Cloud Orchestrator (Gemini 3.7 Flash - High Reasoning & Shadow Auditor)
  2. Local AI Orchestrator (DeepSeek-R1 / Qwen3.8-VL on 7-Layer Mesh)
...
def get_workspace_root() -> Path:
    ...
    if (repo_root / "data").exists() or (repo_root / ".agents").exists():
        return repo_root
    for candidate in [Path("/Users/aaron/DFS_UNIFIED/Lauburu-Monorepo"), Path("/Users/aaron/DFS_UNIFIED/Lauburu-Monorepo")]:
        if candidate.exists():
            return candidate
    return repo_root
...
def build_debate_transcript() -> DebateTranscript:
    """Constructs the complete 4-turn deliberative debate matching verified orchestrator positions."""
    ...
    hardware_context = {
        "core_nodes": {
            "Mac_Node": {"hardware": "Apple M4 Pro Mac Mini", "ram_gb": 16.0, "ram_cap_gb": 12.0, "ai_role": "Host Governor & AutoFS Client"},
    ...
    turn2 = DebateTurn(
        ...
        model_identifier="DeepSeek-R1 / Qwen3.8-VL on 7-Layer Mesh",
    ...
    synthesized_priorities = [
        ...
        "Priority 5: Execute 7-Tier Swarm Truth Audit validating multi-master replication, atomic locking, and zero-hallucination compliance across all 6 mesh devices."
    ]
