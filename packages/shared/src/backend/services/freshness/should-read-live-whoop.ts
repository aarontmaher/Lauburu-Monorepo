...
import type {
  ...
  FreshnessDecisionResult,
} from '../../contracts/freshness.types';

/**
 * Evaluate whether a live WHOOP read is allowed.
 * Pure function — no network calls, no side effects.
 */
export function shouldReadLiveWhoop(
  input: FreshnessDecisionInput,
  nowIso?: string,
): FreshnessDecisionResult {
  ...
  if (input.requiredFields.length > 0) {
    const missing = input.requiredFields.filter(
      (f) => !input.cachedPresentFields.includes(f),
    );
    if (missing.length > 0) {
      return {
        liveReadAllowed: true,
  ...
  if (input.sourceHealth) {
    ...
    if (sh.upstreamStatus === 'down' || sh.upstreamStatus === 'degraded') {
      return {
        liveReadAllowed: true,
        reason: `Source health is ${sh.upstreamStatus}: ${sh.degradedReason ?? 'unknown reason'}.`,
