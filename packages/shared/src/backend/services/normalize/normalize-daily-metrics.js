...
exports.normalizeWhoopDay = normalizeWhoopDay;
exports.normalizeWhoopBatch = normalizeWhoopBatch;
 ...
 */
function normalizeWhoopDay(raw) {
    ...
    const activeCaloriesKcal = raw.dayKilojoules != null
        ...
        : null;
    // Grappling detection from sport names
    const grapplingCount = raw.workoutDetails.filter((w) => GRAPPLING_SPORTS.has(w.sportName.toLowerCase())).length;
    ...
    const completeness = missingFields.length === 0 ? 'complete'
        : presentFields.length >= EXPECTED_FIELDS.length * 0.6 ? 'partial'
            : 'minimal';
    // Source age
    const sourceAgeHours = raw.sourceUpdatedAt
        ? Math.max(0, (Date.now() - new Date(raw.sourceUpdatedAt).getTime()) / 3600000)
        : null;
    // Provenance
    const sourceRefs = [{
            layer: 'raw',
 ...
 */
function normalizeWhoopBatch(rawRecords) {
    return rawRecords
