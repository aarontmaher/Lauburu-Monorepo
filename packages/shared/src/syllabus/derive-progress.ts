...
import type {
  SyllabusBelt,
  SyllabusRequirement,
  AthleteSyllabusProgress,
  AthleteSyllabusItemProgress,
  SyllabusItemStatus,
  BeltLevel,
} from './types';

type ProgressStatus = 'none' | 'drilling' | 'learned' | 'tracking';
 ...
 */
export function deriveSyllabusProgress(
  athleteId: string,
  belt: SyllabusBelt,
  progressMap: Record<string, ProgressStatus>,
): AthleteSyllabusProgress {
  const now = new Date().toISOString();
  const items: AthleteSyllabusItemProgress[] = belt.requirements.map((req) => {
...
function mapRequirementToProgress(
  req: SyllabusRequirement,
  progressMap: Record<string, ProgressStatus>,
): { status: SyllabusItemStatus; notes: string | null; derived: boolean } {
...
function refStatusToSyllabusStatus(refStatus: ProgressStatus): SyllabusItemStatus {
  switch (refStatus) {
