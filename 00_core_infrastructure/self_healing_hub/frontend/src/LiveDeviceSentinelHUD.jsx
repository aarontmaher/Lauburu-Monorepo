...
/**
 * Custom hook for live WebSocket telemetry stream with exponential backoff and 30-sample history.
 */
      ...
      try {
        ...
        ws.onmessage = (event) => {
          ...
          try {
            ...
            const data = (frame.type === 'telemetry_frame' && frame.data) ? frame.data : frame;
            if (data && (data.cpu_usage_pct !== undefined || data.cpu !== undefined)) {
              setTelemetry(data);

              const timeLabel = new Date().toLocaleTimeString([], { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' });
              const cpuVal = data.cpu_usage_pct ?? data.cpu?.usage_pct ?? 0;
              const ramVal = data.ram_usage_pct ?? data.ram?.usage_pct ?? 0;
              const thermVal = data.thermal_celsius ?? data.thermal?.thermal_c ?? 0;
              const gpuVal = data.gpu_usage_pct ?? data.gpu?.usage_pct ?? 0;

              // Append to rolling 30-sample history buffers
              setHistory(prev => ({
                cpu: [...prev.cpu.slice(-29), { time: timeLabel, value: typeof cpuVal === 'number' ? round1(cpuVal) : 0 }],
                ram: [...prev.ram.slice(-29), { time: timeLabel, value: typeof ramVal === 'number' ? round1(ramVal) : 0 }],
                thermal: [...prev.thermal.slice(-29), { time: timeLabel, value: typeof thermVal === 'number' ? round1(thermVal) : 0 }],
                gpu: [...prev.gpu.slice(-29), { time: timeLabel, value: typeof gpuVal === 'number' ? round1(gpuVal) : 0 }]
              }));
            }
          } catch (err) {
 ...
 */
function TelemetrySparkline({ data, strokeColor = '#38bdf8', yDomain = [0, 100], unit = '%', height = 22 }) {
  if (!data || data.length === 0) {
    return (
      <div style={{ height, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.45rem', color: '#64748b' }}>
        -- Awaiting 1Hz Stream --
      </div>
    );
  }
          ...
          <Tooltip
            content={({ active, payload }) => {
              if (active && payload && payload.length) {
                return (
                  <div style={{ background: '#090d16', border: `1px solid ${strokeColor}`, padding: '2px 4px', borderRadius: 2, fontSize: '0.5rem', color: '#f8fafc', zIndex: 100 }}>
...
export default function LiveDeviceSentinelHUD() {
  const { telemetry: liveTelemetry, isConnected: isWsConnected, history: telemetryHistory } = useLiveTelemetry(8000);
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.3rem', borderBottom: '1px solid rgba(255,255,255,0.06)', paddingBottom: '0.2rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
            <span style={{
              ...
              border: isWsConnected ? '1px solid #10b981' : '1px solid rgba(245,158,11,0.3)',
              padding: '1px 5px',
              borderRadius: '3px',
              fontWeight: 'bold',
              display: 'inline-flex',
              ...
              <span>{isWsConnected ? '🟢 1Hz STREAM' : '🟡 WS RECONNECTING'}</span>
            </span>
            ...
            <span style={{ fontSize: '0.58rem', color: '#34d399', background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.25)', padding: '1px 5px', borderRadius: '3px', fontWeight: 'bold' }}>
              🔋 {powerThermalAnalysis.ac_powered_count || 7}/7 Powered • {liveTelemetry?.thermal_celsius ? `${liveTelemetry.thermal_celsius}°C (L1)` : (powerThermalAnalysis.avg_temp_c ? `${powerThermalAnalysis.avg_temp_c}°C Avg` : '33.4°C Avg')}
            </span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '0.2rem' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.5rem', color: '#94a3b8' }}>
              <span>RAM: <strong style={{ color: '#38bdf8' }}>{liveTelemetry?.ram?.used_gb ?? devices.layer1_host_mac?.ram_used_gb ?? 12.5}/{liveTelemetry?.ram?.total_gb ?? 24}GB</strong></span>
            ...
            <div style={{ width: '100%', height: '2px', background: 'rgba(255,255,255,0.1)', borderRadius: '1px', overflow: 'hidden' }}>
              <div style={{ width: `${liveTelemetry?.ram_usage_pct ?? devices.layer1_host_mac?.ram_percent ?? 52}%`, height: '100%', background: 'linear-gradient(90deg, #10b981, #38bdf8)' }} />
            </div>
            {/* Live Real-Time CPU & GPU Sparkline */}
            <div style={{ marginTop: '2px', background: 'rgba(0,0,0,0.25)', borderRadius: '3px', padding: '1px 2px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.44rem', color: '#94a3b8', padding: '0 2px' }}>
                <span style={{ color: '#38bdf8', fontWeight: 'bold' }}>CPU: {liveTelemetry?.cpu_usage_pct ?? '--'}%</span>
                <span style={{ color: '#a78bfa', fontWeight: 'bold' }}>GPU: {liveTelemetry?.gpu_usage_pct ?? liveTelemetry?.gpu?.usage_pct ?? '--'}%</span>
              </div>
              <TelemetrySparkline data={telemetryHistory.cpu} strokeColor="#38bdf8" yDomain={[0, 100]} unit="%" height={20} />
            </div>
