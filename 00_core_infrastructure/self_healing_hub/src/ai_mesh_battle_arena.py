...
- Free-For-All (FFA) Mode across all 13 models on the 5-layer physical hardware mesh.
...
def get_monorepo_root() -> str:
    candidates = [
        "/Volumes/aaronmaher/Lauburu-Monorepo",
    ...
    for c in candidates:
        if os.path.exists(c):
            return c
    return "/Volumes/aaronmaher/Lauburu-Monorepo"
...
DATA_TRANSFER_TRANSPORTS = [
    ...
    },
    {
        ...
        "desc": "Injects sharded RPC layer requests across the 5-layer overlay VPN to siphon distributed compute tokens."
    },
    {
...
EDGE_SOFTWARE_UPGRADES = [
    ...
    },
    {
        ...
        "desc": "Distributes heavy AI inference and truth audit tasks asynchronously across the 5-layer physical mesh."
    },
    {
...
EDGE_DEVICES_CONFIG = {
    "mac_node_host": {
        "id": "mac_node_host",
        "device_name": "Layer 1: Mac Host (M4 Max)",
        "orchestrator_name": "Host Orchestrator (Mac M4 Max)",
        "ip_address": "192.168.8.116 / 100.103.212.21",
        "os": "macOS Sequoia (Darwin 24.6.0)",
        "hardware": "Apple M4 Max (16C CPU, 40C GPU, 16GB Unified RAM)",
...
FACTIONS = {
    FACTION_LOCAL_MESH: {
        ...
        "color": "#10b981",
        "badge": "🟢 LOCAL MESH",
        "motto": "Zero-Latency On-Premises Compute • 54.65 GB Metal GPU / Edge TPU Pool • $0 Recurring API Spend",
        "base_latency_ms": 0.27,
        "special_ability": "⚡ 5-Layer Hardware Sharded Blitz",
        "egress_fee_pct": 0.0
    },
...
AGENTS_ROSTER = [
    # --- 🟢 TEAM LOCAL AI MESH SWARM (ON-PREM EDGE CHAMPIONS) ---
    {
        ...
        "name": "DeepSeek-R1-32B (Mac M4 Max Host)",
        "faction": FACTION_LOCAL_MESH,
        "color": "#06b6d4",
        "node": "Layer 1: This Mac 1 (Primary Orchestrator)",
        "os": "macOS Sequoia / Apple M4 Max Metal",
        "default_lang": "Dart / Python / C++",
        "hardware_tier": "Apple M4 Max (16-core CPU, 40-core Metal GPU)",
    ...
    },
    {
        ...
        "faction": FACTION_LOCAL_MESH,
        "color": "#10b981",
        "node": "Layer 1: Distributed 5-Layer MoE Router",
        "os": "SmolLM2 C-Runtime / 4-Expert MoE Swarm",
        "default_lang": "Python / Shell / Dart / C++",
        "hardware_tier": "Pooled Edge-to-Host (SmolLM2-135M Base + Tensor G5 + M4 Max)",
...
class MeshBattleArena:
    ...
    def init_default_state(self) -> Dict[str, Any]:
        state = {
            ...
            "researched_devices_registry": [
                {"name": "Apple M4 Max Host", "ram_gb": 16.0, "ai_vram_cap_gb": 12.0, "npu_tops": 38.0, "latency_metal_ms": 0.045, "primary_local_ai": "DeepSeek-R1-32B", "tier": "Layer 1 Primary Orchestrator"},
    ...
    def transmigrate_ai_to_device(self, agent_id: str, target_device_name: str) -> Dict[str, Any]:
        ...
        across the 5-layer mesh onto a new physical device (e.g. Mac M4 Max Host, MacBook Pro, Linux, Pixel 10 Pro, Samsung S20).
        """
        agent = next((a for a in self.state.get("agents", []) if self.aid(a) == agent_id or a.get("name") == agent_id), None)
        if not agent:
            return {"success": False, "error": f"Agent {agent_id} not found"}
    ...
    def execute_game_turn(self, action_mode="auto"):
        ...
        if turn_type == "REMOTE_DEVICE_CYBER_HACK" and len(self.state["agents"]) > 1:
            devices = ["Mac_Node (Host M4 Max)", "MacBook_Pro (Worker i7)", "Linux_Head_Node (Ryzen 7)", "Pixel_10_Pro_XL (Tensor G5)", "Samsung_S20 (Exynos 990)", "Gemini Cloud Pod Alpha"]
        ...
        if turn_type == "TRANS_DEVICE_MIGRATION":
            devices = ["Mac_Node (Host M4 Max)", "MacBook_Pro (Worker i7)", "Linux_Head_Node (Ryzen 7)", "Pixel_10_Pro_XL (Tensor G5)", "Samsung_S20 (Exynos 990)"]
        ...
        REAL_OUTLIER_TASKS = [
            {
                ...
                "description": "Sub-millisecond tensor synchronisation across Mac M4 Max Host and Mac Pro Worker over 0.27ms TB4 bridge",
                "base_reward": 75,
    ...
    def get_genie_spatial_world(self):
        ...
        monoliths = [
            {
                ...
                "name": "Citadel 1: Apple M4 Max Host",
                "short_name": "M4 Max Host",
        ...
        monolith_pos_map = {
            "monolith_layer1_mac_host": {"x": -420, "y": 0, "z": -260, "name": "Apple M4 Max Host", "height": 220, "width": 80},
    ...
    def dispatch_genie_action(self, agent_id, action_type, params=None):
        ...
        if not agent:
            agent = active_agents[0] if active_agents else {"name": "DeepSeek-R1-32B (Mac M4 Max Host)", "node": "Mac_Node"}
