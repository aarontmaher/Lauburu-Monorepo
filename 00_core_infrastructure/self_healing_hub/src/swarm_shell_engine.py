...
class SwarmShellSession:
    ...
    async def handle_factcheck(self):
        ...
        await self.send_text(f"  │ \x1b[1m🍎 Primary Mac (M4)\x1b[0m    │ RAM: {mac_dash.get('ram_used_gb', '8.4')}GB / CPU: {mac_dash.get('cpu_load_percent', '18')}% │ M4 Max Darwin Kernel   │ {mac_status:<30}│\r\n")
    ...
    async def handle_shards_audit(self):
        """Scans port 50052 across the full 5-layer network topology."""
        await self.send_text("\r\n\x1b[1;35m🦙 [LLAMA.CPP 5-WAY DISTRIBUTED RPC SHARDS AUDIT]\x1b[0m\r\n")
        await self.send_text("  Auditing 82.8 GB pooled AI VRAM across all 5 physical layers...\r\n\r\n")

        shards = [
            ("Layer 1: Mac Host (M4 Max)", "127.0.0.1", "12.0 GB Cap", "Metal GPU / Orchestrator"),
            ("Layer 2: MacBook Pro (i7)", "100.103.212.21 (TB4: 169.254.187.138)", "12.0 GB Cap", "10Gbps Thunderbolt 4 Bridge"),
            ("Layer 3: Linux Hub (Ryzen 7)", "192.168.8.224:50052", "11.25 GB Cap", "NVMe Cache & Gateway"),
            ("Layer 4: Pixel 10 Pro XL", "100.73.38.87:50052", "11.4 GB Cap", "Tensor G5 + Edge TPU"),
            ("Layer 5: Samsung S20+", "100.84.40.95:50052", "8.0 GB Cap", "Exynos 990 Worker")
        ]
