...
| FS-021 | Lactate testing + Daily Dozen-style checklist + expanded nutrition/recovery correlation metrics | feature_idea | candidate | Per `docs/FS021_HEALTH_INPUTS_EXPANSION_SPEC.md`. Lactate is manual/CSV-first training/performance context (mmol/L, protocol, stage/workload, HR, RPE, timing, source, confidence); BLE/device protocol investigation is a later lane. Daily Dozen-style checklist is a manual nutrition-habit context, not assumed to have an official API/export. Pattern layer may compare macro ratios, grams, carb timing, checklist completion, and recovery/training context with association-only language. No medical advice, no causation claims, no clinical interpretation. |
...
FS-021 | 2026-05-08 candidate recorded by Aaron — lactate manual/CSV-first + Daily Dozen-style checklist + association-only nutrition/recovery pattern layer. Awaiting batch approval after current release gate / FS-020 parser confirmation.
```
