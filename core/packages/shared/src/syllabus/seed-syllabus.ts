...
import type { Syllabus, SyllabusRequirement, BeltLevel } from './types';

function req(
  id: string,
  label: string,
  opts: Partial<SyllabusRequirement> = {},
): SyllabusRequirement {
  return {
    id,
    label,
...
function techReq(
  ...
  heading: string,
  technique: string,
  priority: SyllabusRequirement['priority'] = 'core',
): SyllabusRequirement {
  return req(id, label, {
    type: 'technique',
    priority,
...
export const SEED_SYLLABUS: Syllabus = {
  schemaVersion: 1,
...
for (const belt of SEED_SYLLABUS.belts) {
