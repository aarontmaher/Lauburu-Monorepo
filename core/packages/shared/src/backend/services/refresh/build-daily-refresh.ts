...
export interface BuildDailyRefreshInput {
  /** Today's normalized record. */
  today: NormalizedDailyMetrics;
  /** Last 3 days of normalized records (for rolling risk). Includes today. */
  recent3Days: NormalizedDailyMetrics[];
  /** Athlete's stable baseline from Layer 4. */
  baseline: AthletePhysiologyBaseline;
  /** Config overrides (optional — defaults to canonical config). */
  engineConfig?: AthleteDailyDecisionEngineConfig;
  riskConfig?: AthleteRolling3DayRiskModel;
}
...
function compareToBaseline(
  ...
  let status: DailyRefreshFact['status'] = 'normal';
  if (absD > 10) status = isWorse ? (isSuppressed ? 'suppressed' : 'elevated') : 'normal';
  else if (absD > 5) status = isWorse ? (isSuppressed ? 'suppressed' : 'elevated') : 'normal';
...
function classifyDay(
  ...
  config: AthleteDailyDecisionEngineConfig,
  today: NormalizedDailyMetrics,
  baseline: AthletePhysiologyBaseline,
): { state: DayState; score: number; reasons: string[] } {
  let score = 50;
  const reasons: string[] = [];
  const t = config.thresholds;

  function ratio(current: number | null, base: number | null): number | null {
  ...
  if (recoveryR != null) {
    if (recoveryR <= t.recoveryBelowBaseline.warningPct) { score -= 20; reasons.push('Recovery materially below baseline'); }
    else if (recoveryR <= t.recoveryBelowBaseline.cautionPct) { score -= 10; reasons.push('Recovery below baseline'); }
    else if (recoveryR >= 1.05) { score += 10; reasons.push('Recovery above baseline'); }
  }

  const hrvR = ratio(today.hrvMs, baseline.hrvBaseline);
  if (hrvR != null) {
    if (hrvR <= t.hrvBelowBaseline.warningPct) { score -= 15; reasons.push('HRV materially suppressed'); }
    else if (hrvR <= t.hrvBelowBaseline.cautionPct) { score -= 8; reasons.push('HRV below baseline'); }
  }

  const rhrR = ratio(today.restingHrBpm, baseline.restingHrBaseline);
  if (rhrR != null) {
    if (rhrR >= t.restingHrAboveBaseline.warningPct) { score -= 15; reasons.push('Resting HR materially elevated'); }
    else if (rhrR >= t.restingHrAboveBaseline.cautionPct) { score -= 8; reasons.push('Resting HR elevated'); }
  }

  const sleepR = ratio(today.totalSleepHours, baseline.sleepBaseline);
  if (sleepR != null) {
    if (sleepR <= t.sleepBelowBaseline.warningPct) { score -= 12; reasons.push('Sleep materially below baseline'); }
    else if (sleepR <= t.sleepBelowBaseline.cautionPct) { score -= 6; reasons.push('Sleep below baseline'); }
    else if (sleepR >= 1.03) { score += 5; reasons.push('Sleep above baseline'); }
  }

  score = Math.max(0, Math.min(100, score));
  const state: DayState =
    score >= config.greenCutoff ? 'green'
...
function computeRollingRisk(
  recent: NormalizedDailyMetrics[],
  baseline: AthletePhysiologyBaseline,
  config: AthleteRolling3DayRiskModel,
  thresholds: AthleteDailyDecisionEngineConfig['thresholds'],
): { level: RiskLevel; score: number; drivers: string[] } {
  let score = 0;
  const drivers: string[] = [];
  const window = recent.slice(-config.windowDays);

  function countBelow(extract: (d: NormalizedDailyMetrics) => number | null, base: number | null, threshold: number): number {
  ...
  function countAbove(extract: (d: NormalizedDailyMetrics) => number | null, base: number | null, threshold: number): number {
    if (base == null) return 0;
    return window.filter((d) => {
      const v = extract(d);
      return v != null && v / base >= threshold;
...
function buildSessionGate(
  dayState: DayState,
  riskLevel: RiskLevel,
): DailyRefreshArtifact['sessionGate'] {
  if (riskLevel === 'high') {
...
function buildRecommendation(
  ...
  const guidanceMap: Record<DayState, string> = {
 ...
 */
export function buildDailyRefreshArtifact(
  input: BuildDailyRefreshInput,
): DailyRefreshArtifact {
  const { today, recent3Days, baseline } = input;
  const config = input.engineConfig ?? ATHLETE_DAILY_DECISION_ENGINE_CONFIG;
  const riskConfig = input.riskConfig ?? ATHLETE_ROLLING_3_DAY_RISK_MODEL;
  const now = new Date().toISOString();

  // Baseline-compared facts (THIS is where baseline flags live)
  const facts: DailyRefreshFact[] = [
  ...
  const activePatterns: DailyRefreshArtifact['activePatterns'] = [];
  ...
  // Warning flags
  const warningFlags: DailyRefreshArtifact['warningFlags'] = [];
  if (risk.level === 'high') warningFlags.push({ level: 'warning', label: 'High rolling risk' });
  if (risk.level === 'elevated') warningFlags.push({ level: 'watch', label: 'Elevated rolling risk' });
