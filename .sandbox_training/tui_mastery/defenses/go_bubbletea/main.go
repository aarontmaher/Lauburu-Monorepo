...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	// Panic boundary recovery
	defer func() {
		if r := recover(); r != nil {
			m.err = fmt.Errorf("recovered from panic in Update: %v", r)
		}
	}()
	...
	case tea.WindowSizeMsg:
		...
		m.height = max(msg.Height, 5)
		m.table.SetWidth(max(msg.Width-6, 10))
...
func (m Model) View() string {
	...
	defer func() {
		if r := recover(); r != nil {
			_ = fmt.Sprintf("View panic recovered: %v", r)
		}
	}()
